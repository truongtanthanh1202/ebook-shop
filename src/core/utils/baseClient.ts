...
export class BaseClient {
  ...
  create({ setAuthorizationFn }: ConfigInstance = {}) {
    const defaultSetAuthorizationFn = (config: InternalAxiosRequestConfig) => {
    ...
    api.interceptors.response.use(
      ...
      async (error: AxiosError) => {
        ...
        if (isNotifyWhenFail(errorResponse) && LIST_CODE_ERROR.includes(statusCode)) {
          // showErrorToast(errorResponse.data?.message);
        }
        ...
        this.processQueue(null, res.data?.accessToken); // res.data?.accessToken is new access token
